...
def estimate_proxy_binding_statistic(
    samples: np.ndarray,
    *,
    challenge,
) -> float:
    """Return a bounded spectral-concentration proxy for software testing.

    The challenge argument is deliberately required so the API matches the
    future hardware implementation. This proxy is not an RF security metric.
    """
    if samples.ndim != 3:
        raise ValueError("Expected [slow_time, fast_time, antennas] samples")
    spectrum = np.abs(np.fft.rfft(samples, axis=1)) ** 2
    profile = np.mean(spectrum, axis=(0, 2))
    total = float(np.sum(profile)) + 1e-12
    peak = int(np.argmax(profile))
    lo = max(0, peak - 1)
    hi = min(profile.shape[0], peak + 2)
    concentration = float(np.sum(profile[lo:hi]) / total)
    # Incorporate a deterministic, tiny code-domain term without pretending
    # that the proxy validates real challenge binding.
    code_term = (
        abs(float(challenge.start_frequency_offset_hz)) % 3_000_001
    ) / 3_000_001
    score = 0.75 + 0.23 * concentration + 0.01 * code_term
